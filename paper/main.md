...
by setting `EXACT_RANK_MODE = False` in the script. This evaluates the
substituted matrices $L$ and $[L|b]$ at $20$-digit precision via mpmath
and applies `numpy.linalg.matrix_rank` with tolerance $10^{-10}$. This
mode is non-binding: it serves only to identify candidate consistent
cases quickly, after which the `EXACT_VERIFY_SAMPLE` flag confirms each
